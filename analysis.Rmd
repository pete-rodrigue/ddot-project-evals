...
to_plot <-
  ...
  dplyr::group_by(`Project short name`, Outcome) %>%
  dplyr::summarize(change = mean(Change, na.rm=T)) %>%
  ungroup() %>%
...
for (proj in unique(cd_in_projs_summed$project_name)) {  
  ...
  plot_table <-
    rvs[[3]] %>% 
      dplyr::left_join(
        select(df, `Project name`, Outcome, Change) %>% 
          filter(`Project name` == proj) %>%
          mutate(Outcome = if_else(Outcome == "Any Injury", "Injury Crashes", Outcome)), 
        by=c("series"="Outcome")) %>%
...
```{r}
bl_summary <- data.frame("Project" = NA, "Outcome" = NA, "% change" = NA, "MM change" = NA, "Before" = NA, "After" = NA)
bl_objs    <- list()
for (proj in unique(cd_in_projs_summed$route_unique_id)) {  
  ...
  mm_data <- 
    ...
    dplyr::summarize(mean_2019_trips  = round(mean(mean_2019_trips , na.rm=T)), 
  ...
  bl_summary <- bind_rows(
    bl_summary, 
    data.frame("Project" = rep(proj, 4), 
               "Outcome" = c("All Crashes", "Injury Crashes", "Serious Injury Crashes", "Fatal Crashes"), 
               "% change" = c(plot_table$`% change`[plot_table$Outcome == "All Crashes"],
                              plot_table$`% change`[plot_table$Outcome == "Injury Crashes"],
                              plot_table$`% change`[plot_table$Outcome == "Serious Injury Crashes"],
                              plot_table$`% change`[plot_table$Outcome == "Fatal Crashes"]),
               "Before"   = c(plot_table$`Before, per month`[plot_table$Outcome == "All Crashes"],
                              plot_table$`Before, per month`[plot_table$Outcome == "Injury Crashes"],
                              plot_table$`Before, per month`[plot_table$Outcome == "Serious Injury Crashes"],
                              plot_table$`Before, per month`[plot_table$Outcome == "Fatal Crashes"]),
               "After"    = c(plot_table$`After, per month`[plot_table$Outcome == "All Crashes"],
                              plot_table$`After, per month`[plot_table$Outcome == "Injury Crashes"],
                              plot_table$`After, per month`[plot_table$Outcome == "Serious Injury Crashes"],
                              plot_table$`After, per month`[plot_table$Outcome == "Fatal Crashes"])
               )
  )
}
...
to_show_1 <-
  ...
  group_by(Outcome) %>%
  summarise(
    before_per_year  = sum(Before, na.rm=T)*12,
    after_per_year   = sum(After, na.rm=T)*12,
    mean_pct_change       = mean(`X..change`, na.rm=T),
    pooled_pct_change = (after_per_year - before_per_year) / before_per_year
  )



before <- setNames(to_show_1$before_per_year, to_show_1$Outcome)
after  <- setNames(to_show_1$after_per_year,  to_show_1$Outcome)

to_show_2 <- tribble(
  ~Outcome,                     ~before_per_year,                                                  ~after_per_year,
  "Property Destruction Only",  before["All Crashes"] - before["Injury Crashes"] - before["Fatal Crashes"], after["All Crashes"] - after["Injury Crashes"] - after["Fatal Crashes"],
  "Fatal Crashes",              before["Fatal Crashes"],                                                after["Fatal Crashes"],
  "More Minor Injury Crashes",  before["Injury Crashes"] - before["Serious Injury Crashes"],             after["Injury Crashes"] - after["Serious Injury Crashes"],
  "Serious Injury Crashes",     before["Serious Injury Crashes"],                                        after["Serious Injury Crashes"]
)

to_show_1
to_show_2 %>% mutate(reduction = after_per_year - before_per_year)


bl_summary <-
  bl_summary %>%
  select(-Before, -After) %>%
  filter((Outcome != "Serious Injury Crashes") & (Outcome != "Fatal Crashes")) %>%
  filter(!(Project %in% already_shown))
```
