...
for (proj in unique(cd_in_projs_summed$route_unique_id)) {  
  ...
  pbl_flag <- ifelse(is_pbl, " [PBL]", "[BBL]")
  ...
  pbl_objs <- c(pbl_objs, list(list(proj, is_pbl, rvs[[1]], plot_table)))
...
for (proj in unique(cd_in_projs_summed$route_unique_id)) {  
  ...
  buffered_objs <- c(buffered_objs, list(list(proj, is_pbl, rvs[[1]], plot_table)))
...
panel_pairs <- list(
  ...
  "Driver"      = list(total = "Vehicle Crashes per 800 meters",  injury = "Driver Injury per 800 meters"),
  "Walker"  = list(total = "Pedestrian Crashes per 800 meters", injury = "Pedestrian Injury per 800 meters"),
  "Biker"        = list(total = "Bike Crashes per 800 meters",     injury = "Bicyclist Injury per 800 meters")
)
...
```{r, eval=F}
outcomes <- c("Total Crashes per 800 meters", "Any Injury per 800 meters", 
                         "Vehicle Crashes per 800 meters", "Driver Injury per 800 meters",
                         "Bike Crashes per 800 meters", "Bicyclist Injury per 800 meters",
                         "Pedestrian Crashes per 800 meters", "Pedestrian Injury per 800 meters")
...
summary_df <-
  crashes_near_subblocks_filled |>
  select(SUBBLOCKKEY, post, treated, ends_with(" per 800 meters")) |>
  pivot_longer(cols = all_of(outcomes),
               names_to  = "outcome",
               values_to = "value") |>
  group_by(outcome, post, treated) |>
  summarise(
