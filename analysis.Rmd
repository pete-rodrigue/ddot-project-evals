...
pbl <- st_read("labeled_bike_lanes/Protected-Buffered-Bike-Lanes-labeled.geojson", quiet=T) %>%
  filter(!is.na(install_year)) %>% rename(build_year = install_year) #%>%
...
rs <- 
  st_read("Roadway_SubBlock.geojson", quiet=T) %>%
...
cd_pbl_clean <- cd_pbl_clean %>%
  ...
  filter(year %in% c(2015, 2016, 2017, 2018, 2019, 2024, 2025)) %>%
