...
cd <- readr::read_csv("data/Crashes_in_DC.csv")
...
cabi <- readr::read_csv("data/cabi_both_years_clean.csv")
...
rs_radius <- (rs$TOTALPARKINGLANES + rs$TOTALTRAVELLANES)*10 / 2 + 10
rs_crash_buf <-
  rs |>
  st_transform(6487) |>          # EPSG:6487 — NAD83 / Maryland (metres)
  st_buffer(rs_radius) |>        # X m buffer
  st_transform(st_crs(cd)) |>    # reproject back to match cd's CRS
...
# add very rough CaBi numbers by year:
annual_cabi_rides <-
  data.frame(
    "year"       = c(2016, 2017, 2018, 2019, 2024, 2025),
    "cabi_rides" = c(3.5 , 3.8 , 3.4 , 3.4 , 6.1 , 6.6)
    )
